...
def find_mesh_quality(tet_file):
    ...
    W = np.array(
        [[1, 0.5, 0.5], [0, np.sqrt(3) / 2, np.sqrt(3) / 6], [0, 0, np.sqrt(2 / 3)]]
    )
    Winv = np.linalg.inv(W)
    A = lambda e: np.vstack(
        (
            nodes[e[1]] - nodes[e[0]],
            nodes[e[2]] - nodes[e[0]],
            nodes[e[3]] - nodes[e[0]],
    ...
    kappa = lambda A: frob_norm(np.matmul(A, Winv)) * frob_norm(
        np.matmul(W, np.linalg.inv(A))
    )
