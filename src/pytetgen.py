...
def call_tetgen(file, args="-d"):
    ...
    if platform == "win32":
        tetgen_cmd = f"{tetgen_base_path}/tetgen/win64/tetgen.exe"
    ...
    if platform != "linux" and platform != "linux2" and platform != "darwin":
        ...
        # Move_command = f'mv {os.path.splitext(os.path.basename(file))[0:2]}.* {os.path.dirname(file)}/'
        print(f"{tetgen_cmd} {args} {os.path.abspath(file)};\ncd {cwd};")
        output = os.popen(
            f"{tetgen_cmd} {args} {os.path.abspath(file)};\ncd {cwd};"
        ).read()
    print(output)
