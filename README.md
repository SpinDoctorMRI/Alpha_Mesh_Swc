...
```
python SkeletonMeshError.py output/04b_spindle3aFI.ply input/04b_spindle3aFI.swc output/04b_spindle3aFI.txt --save_pc=1;
python SkeletonMeshError.py output/1-2-1.CNG.ply input/1-2-1.CNG.swc output/1-2-1.CNG.txt --save_pc=1;
```
The Skeleton to Mesh Error is the RMS value in the output file. To visualize the full distribution of the scalar field d, run
...
python view_outputs/view_error.py output/1-2-1.CNG.ply output/1-2-1.CNG_pc.ply;
```

### Microglia scrips
To create the microglia meshes from the mixed cell descriptions run:
```bash
mkdir Microglia/Amoeboid_output;
for file in Microglia/Amoeboid/*.swc; do
    cellname="${file%.*}";
    echo "Meshing $cell";
    python mesh_mixed_swc.py $cellname --log=1 --output_dir=Microglia/Amoeboid_output --alpha=0.001;
done
...
for file in Microglia/Ramified/*.swc; do
    cellname="${file%.*}";
    echo "Meshing $cell";
    python mesh_mixed_swc.py $cellname --log=1 --output_dir=Microglia/Ramified_output --alpha=0.001;
done
```
...
for file in Microglia/Amoeboid_output/*.ply; do
    ...
    cellname="${meshname%.*}";
    python SkeletonMeshError.py $file "Microglia/Amoeboid/"$cellname".swc" "Microglia/Amoeboid_output"/$cellname".txt" --soma_mesh="Microglia/Amoeboid/"$cellname".wrl" --save_pc=1;
done

for file in Microglia/Ramified_output/*.ply; do
    meshname=$(basename $file);
    cellname="${meshname%.*}";
    python SkeletonMeshError.py $file "Microglia/Ramified/"$cellname".swc" "Microglia/Ramified_output"/$cellname".txt" --soma_mesh="Microglia/Ramified/"$cellname".wrl" --save_pc=1;
done
```
Removing the --save_pc=1 flag will not save the point clouds.


### Batch scripts
Batch mesh generations on the dataset of 1000 neurons is done by:
```
    python batch_mesh_swc.py 1000_swc_files --output_dir=1000_surface_meshes
```
Optional parameters are the same as for mesh_swc.py.

The Skeleton to Mesh Error can be computed with:
```
    python batch_SkeletonMeshError.py 1000_surface_meshes 1000_swc_files 1000_SME     
```
To store meshing data from every mesh into a single database file run
```
    python get_data.py 1000_surface_meshes Alpha_Mesh_Swc_1000_Neurons_summary.db --SME_dir=1000_SME   
```
To run TetGen and test the quality of finite element meshes run
```
    python check_femeshes.py 1000_surface_meshes 1000_FE_data Alpha_Mesh_Swc_1000_Neurons_summary.db 
```
This stores the quality and size of the finite element meshes in a separate table in Alpha_Mesh_Swc_1000_Neurons_summary.db. 
All this data can then be viewed in the .db file directly or by using plot_summary_data.ipynb.

### Diffusion MRI simulations
The scripts used for diffusion MRI simulations are available here https://github.com/SpinDoctorMRI/SpinDoctor/tree/Paper_NeuronMeshing.

### Modified Ultraliser scripts
See Modified_Ultraliser/README.md.
